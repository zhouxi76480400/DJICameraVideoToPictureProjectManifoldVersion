...
void rgbtobgr(uint8_t * img,uint8_t * data,int lineSize,int h);

bool convertYV12ToBGR24(uint8_t * pYUV, uint8_t * pBGR24_fin,int width,int height) {
    if (width < 1 || height < 1)
        return false;
    ...
    rgbtobgr(pBGR24_fin,pBGR24,file_size / height,height);
    delete [] pBGR24;
    return true;
}
